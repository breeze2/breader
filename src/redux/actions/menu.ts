...
export interface ISetMenuKeyPayload {
  key: string
}
export const setMenuKeyAction = (key: string) =>
  actionCreator<ISetMenuKeyPayload>(MenuActionTypes.SET_MENU_KEY)({ key })

export interface IAsyncSelectMenuKeyPayload {
  key: string
}
export const asyncSelectMenuKeyAction = (key: string) =>
  actionCreator<IAsyncSelectMenuKeyPayload>(
    MenuActionTypes.ASYNC_SELECT_MENU_KEY
  )({ key })
