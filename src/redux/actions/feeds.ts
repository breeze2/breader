...
export const setIsUpdatingFeedsAction = (isUpdating: boolean) =>
  actionCreator<ISetIsUpdatingFeedsPayload>(
    FeedsActionTypes.SET_IS_UPDATING_FEEDS
  )({ isUpdating })

export interface ISetIsCreatingFeedPayload {
  isCreating: boolean
}
export const setIsCreatingFeedAction = (isCreating: boolean) =>
  actionCreator<ISetIsCreatingFeedPayload>(
    FeedsActionTypes.SET_IS_CREATING_FEED
  )({ isCreating })
...
export interface IAsyncParseFeedPayload {
  feedUrl: string
}
export const asyncParseFeedAction = (feedUrl: string) =>
  actionCreator<IAsyncParseFeedPayload>(FeedsActionTypes.ASYNC_PARSE_FEED)({
    feedUrl,
  })

export interface IAsyncDeleteFeedsPayload {
  feedIds: string[]
}
export const asyncDeleteFeedsAction = (feedIds: string[]) =>
  actionCreator<IAsyncDeleteFeedsPayload>(FeedsActionTypes.ASYNC_DELETE_FEEDS)({
    feedIds,
  })
