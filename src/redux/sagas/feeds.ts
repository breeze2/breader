...
export function* fetchFeedsSaga(action: IReduxAction<null>) {
  ...
  if (
    currentArticle &&
    !feeds.some(feed => feed._id === currentArticle.feedId)
  ) {
...
export function* parseFeedSaga(action: IReduxAction<IAsyncParseFeedPayload>) {
  const feed: IFeed | null = yield call(
    Logic.createFeed,
...
export function* deleteFeedsSaga(
  action: IReduxAction<IAsyncDeleteFeedsPayload>
) {
